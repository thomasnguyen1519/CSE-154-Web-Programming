...
/*  Name: Tom Nguyen
    ...
    function deleteOne($db, $pokemon) {
        if (checkOwned($db, $pokemon)) {
            $db->exec("DELETE FROM Pokedex WHERE name='$pokemon'");
        } else {
    ...
    // Accepts a PDO and a Pokemon name param. Returns the name of the Pokemon in the table.
    function getName($db, $pokemon) {
        $rows = $db->query("SELECT name FROM Pokedex WHERE name='$pokemon'");
        foreach ($rows as $row) {
            $nameFixed = $row["name"];
        }
        return $nameFixed;
